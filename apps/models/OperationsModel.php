...
class OperationsModel extends PDOModel
{
    ...
    public function editOperation($opId, $account_id, $amount, $category_id, $payment_method, $comment) {
        $q = "UPDATE accounts SET accountProvision = accountProvision - (SELECT (CASE c.typeCat WHEN 'credit' THEN o.amount ELSE o.amount * -1 END ) FROM operations o, category c WHERE o.id = :idOp AND c.id = o.idCategory ORDER BY `date` DESC LIMIT 1) WHERE idUser = :userId AND id = :idAccount;";
        $this->request($q, array(
            "opId" => (int) $opId,
            "amount" => (float) $amount,
            "accountId" => (int) $account_id,
